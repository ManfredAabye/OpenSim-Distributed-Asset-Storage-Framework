...
internal class MinioClient : IMinioClient
{
    ...
    public async Task<IObservable<NotificationEvent>> ListenBucketNotificationsAsync(string bucketName, IEnumerable<EventType> events, string prefix, string suffix, CancellationToken cancellationToken)
    {
        ...
        {
            var responseBody = await resp.Content.ReadAsStreamAsync(cancellationToken).ConfigureAwait(false);
            return new NotificationEventObservable(resp, responseBody);
        }
        catch
        {
            resp.Dispose();
    ...
    private sealed class NotificationEventObservable : IObservable<NotificationEvent>
    {
        private readonly HttpResponseMessage _response;
        private readonly Stream _responseBody;
        private int _subscribed;

        public NotificationEventObservable(HttpResponseMessage response, Stream responseBody)
        {
            _response = response;
        ...
        public IDisposable Subscribe(IObserver<NotificationEvent> observer)
        {
            ...
            {
                observer.OnError(new InvalidOperationException("Notification stream supports only one subscriber."));
            ...
            _ = Task.Run(async () =>
            {
                ...
                {
                    using (_response)
                    await using (_responseBody.ConfigureAwait(false))
                    {
                        ...
                        while (!sr.EndOfStream)
                        {
                            ...
                            foreach (var record in bucketNotificationEvent.Records)
                            {
                                cts.Token.ThrowIfCancellationRequested();
                ...
                {
                    // Cancellation is initiated by disposing the subscription.
                }
                catch (Exception ex)
                {
                    observer.OnError(ex);
